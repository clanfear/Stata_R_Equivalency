...
poisson y_count x z
```
`poisson` offers more postestimation commands.
...
`prvalue` is a postestimation command, so it must be run immediately after a model. `prvalue` will return the predicted rate (mean) count as well as probability of observing particular counts.
```
glm y_count x z, family(poisson) link(log)
...
```
plot(density(fitted(example_pois)), lty="dashed", col="red")
