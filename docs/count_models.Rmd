...
example_data <- data.frame(MASS::mvrnorm(300,
                                         ...
                                         Sigma = cov_mat,
                                         empirical = TRUE))
example_data$y_count <- with(example_data, rpois(300, 10+x+z))
```

Then the actual prediction and plotting:
```
n_vals    <- 20  # number of x-values to plot over
pr_counts <- c(0,2,5) # counts for which we want probabilities
...
matplot(z_range, pr_values,
        pch = paste0(as.character(pr_counts)))
```
