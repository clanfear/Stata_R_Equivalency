...
```
In Stata we create a vector of means (named `mean_vec` here) and a matrix of covariances (named `cov_mat`). `corr2data` then generates data with variables named `x`, `y`, and `z` whose means (`mean(mean_vec)`) and covariances (`cov(cov_mat)`) match what we provided. We specify the sample size with `n()`.
</div>
