...
Poisson regression is a generalized linear model for count data with an equal mean and variance. Coefficients are reported in log-counts.
</div>
...
```
Working on random poisson generation in Stata
```
</div>
...
First we generate some count data based on covariates, then we model it.

```
example_data$y_count <- with(example_data, 
                             rpois(300, 4 + x + z ))
...
<div class="column-center">
### Count Probabilities for Poisson
...
Still working on Stata equivalence.
</div>
